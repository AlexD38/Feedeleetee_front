...
function ClientsInfos(props) {
    ...
    useEffect(() => {
        async function fetchClients() {
            ...
            const response = await axios.get(`https://feedeleetee-back.vercel.app/enterprises/clients`, { headers });
            ...
            {myClients ? (
                <div className="card">
                    ...
                    {myClients.map((clientInformation) => (
                        <ul className="clients-infos" key={clientInformation.id}>
                            <li key={clientInformation.id}>
                                <SingleCLient className="single-client" client={clientInformation} />
                            </li>
                        </ul>
                    ))}
                </div>
