...
function EnterpriseDashboard() {
    ...
    useEffect(() => {
        async function fetchDashboard() {
            ...
            try {
                const response = await axios.get("https://feedeleetee-back.vercel.app/enterprise/", {
                    headers,
                });
                if (response.data[0]) {
    ...
    const showNav = () => {
        ...
        nav.classList.contains("nav-show") ? nav.classList.remove("nav-show") : nav.classList.add("nav-show");
    };
    ...
    return (
        <>
            {myEnterprise.name && (
                <>
                    <nav>
                        <img className="logo" src={`data:image/png;base64,${myEnterprise.logo}`} alt="logo" />
                        ...
                        <ul id="nav">
                            ...
                            <li onClick={(e) => handleClick(e.target.textContent)}>Services</li> <li onClick={(e) => handleClick(e.target.textContent)}>Mon entreprise</li>{" "}
                        </ul>
                    ...
                    {currentComponent === "Mon entreprise" && <MyEnterprise enterprise={myEnterprise} onUpdate={updateEnterprise} />}
                </>
            )}
        </>
    );
