...
export default function (props) {
    ...
    const uploadLogo = async (e) => {
        ...
        if (logoFile) {
            ...
            const headers = { token };
            try {
                const response = await axios.post("https://feedeleetee-back.vercel.app/logo", data, {
                    headers,
                });
    ...
    const handleClick = async () => {
        ...
        try {
            ...
            if (name.length > 0 && address.length > 0 && description.length > 0) {
                const response = await axios.patch(`https://feedeleetee-back.vercel.app/enterprises/${id}`, { data }, { headers });
