		...
		try {
			...
			if (
				...
				const response = await axios.patch(
					`http://localhost:4000/enterprises/${id}`,
					{ data },
					{ headers }
				);
		...
		<>
			...
			{showInput ? (
				<>
					...
					<input
						ref={nameRef}
						placeholder={myEnterprise.name}></input>
					<label>Address : </label>
					<input
						ref={addressRef}
						placeholder={myEnterprise.address}></input>
					<label>Description : </label>
					<textarea
						ref={descriptionRef}
						placeholder={myEnterprise.description}></textarea>
				</>
			) : (
				<>
