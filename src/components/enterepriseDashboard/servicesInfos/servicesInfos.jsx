...
function ServicesInfos(props) {
    ...
    useEffect(() => {
        async function fetchServices() {
            ...
            const response = await axios.get(`https://feedeleetee-back.vercel.app/enterprises/services`, { headers });
    ...
    const deleteService = async (e) => {
        ...
        try {
            const response = await axios.delete(`https://feedeleetee-back.vercel.app/services/${id}`, {
                headers,
            });
                ...
                <div className="card">
                    ...
                    <ul className="services-container">
                        {myServices.map((serviceInformation) => (
                            <li>
                                <ul className="service-infos" onClick={(e) => handleServiceClick(serviceInformation.id)} id={serviceInformation.id}>
                                    <li className="service-description">{serviceInformation.description} : </li>
                                    <li className="service-price-duration-container">
                                        <p className="service-price">{serviceInformation.price} €</p>
                                        <p className="service-duration">{serviceInformation.duration}H</p>
                                    </li>
                                    {selectedServiceId === serviceInformation.id && (
                                        <button className="del-btn--service" onClick={deleteService} id={selectedServiceId}>
                                            DELETE
                                        </button>
                                    )}
                                </ul>
